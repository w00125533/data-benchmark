...
class SparkIcebergClientTest {
    ...
    void loadBuildsDockerComposeSparkSqlCommandWithHdfsHiveAndSql() {
        ...
        DatasetResult dataset = new DatasetResult(Path.of("data/run-1"), List.of(Path.of("data/run-1/part.parquet")), 42, 128);

        EngineRunResult result = new SparkIcebergClient(runner).load(dataset, "run-1", "smoke");
    ...
    void failedSparkCommandBecomesFailedEngineResultWithStderr() {
        ...
        EngineRunResult result = new SparkIcebergClient(runner)
            .load(new DatasetResult(Path.of("data/run-1"), List.of(), 42, 128), "run-1", "smoke");
